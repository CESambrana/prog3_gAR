...
const pacientesService = {
    ...
    delete: async (id) => {
        const [paciente] = await db.query("SELECT id_usuario FROM pacientes WHERE id_paciente = ?", [id]);
        if (paciente.length === 0) return 0;
        const [result] = await db.query("UPDATE usuarios SET activo = 0 WHERE id_usuario = ?", [paciente[0].id_usuario]);
        return result.affectedRows;
    },
