...
const medicosRepo = {
    ...
    getValorConsulta: async (id_medico) => {
        const [rows] = await db.query(
            "SELECT valor_consulta FROM medicos WHERE id_medico = ?", [id_medico]
        );
        return rows[0] || null;
    },
    ...
    getIdUsuario: async (id) => {
        const [rows] = await db.query(
            "SELECT id_usuario FROM medicos WHERE id_medico = ?", [id]
        );
        return rows[0] || null;
    },
